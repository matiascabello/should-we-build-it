...
**`server/app.py`** — FastAPI app. `POST /api/debate` (JSON body: `question`, `max_rounds`, `context` documents) builds a per-request retriever from the supplied context, then runs the graph via `_drain_graph`, a separate `asyncio.Task` that pushes every `(mode, chunk)` update from `graph.astream(initial, stream_mode=["updates", "custom"])` into a queue. `debate_stream` reads that queue and re-emits each update as an SSE event via `_serialize_update` — the "updates" stream produces `turn`/`flags`/`verdict`/`round`/`rejected` events; the "custom" stream produces a `tool_call` event the moment each one happens, from `graph/nodes.py`'s `stream_writer` calls. Sleeps 0.4s between "updates" events and 0.25s between live tool calls so the frontend renders a "live" unfolding debate rather than a burst.
- **The producer/queue split exists for one reason**: `factcheck_node`/`judge_node` (and a debater composing its final argument after its last tool call) are single blocking LLM calls with no interim event — the SSE connection can go silent for the whole call. Confirmed locally (a proxy that force-closes connections idle beyond a few seconds, put in front of the real app) that an idle-timeout intermediary can kill that "quiet but alive" connection even though the graph is running fine server-side — the client sees it as a network error with a clean LangSmith trace and no server-side error to explain it. `debate_stream` reads the queue with `asyncio.wait_for(..., timeout=HEARTBEAT_INTERVAL_SECONDS)` (15s) and writes an SSE comment line (`: keep-alive`, silently ignored by the frontend's parser) on every timeout — because the graph runs in its own task, a heartbeat timeout only ever abandons a `queue.get()` wait, never the graph itself. On an early client disconnect, the `finally` block cancels the producer task so a debate nobody's listening to doesn't keep spending API calls.
- Rate-limited in-memory (`MAX_CONCURRENT_DEBATES`, `MAX_DEBATES_PER_IP_PER_HOUR`, `MAX_DEBATES_PER_DAY`) since a debate is many real LLM calls, not a cheap request — single-process only, doesn't share state across multiple workers/instances. `GET /` serves `ui/index.html` as a static file (no templating).
