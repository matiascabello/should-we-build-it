...
**`graph/state.py`** — the shared state and every Pydantic model that flows through the graph: `ToolCall` (one evidence-gathering action, with a `url` populated for web results), `Turn` (one debater's argument + its tool calls), `Flag` (fact-checker's verdict on a claim), `DecisionMemo` (judge's final structured output), and `DebateState` (the `TypedDict` LangGraph threads through every node — `transcript` and `flagged_claims` use `Annotated[..., add]` so node returns *append* rather than overwrite).
